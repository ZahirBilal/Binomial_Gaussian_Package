...
# TODO: make a Binomial class that inherits from the Distribution class. Use the specifications below.
    ...
    def __init__(self, prob=0.3, size_of_dist=25):
        self.p = prob
        self.n = size_of_dist
...
#       count = 0 # counter for no of the positive trials in the data set
...
#        self.p = count / self.n
    ...
    def plot_bar(self):
        ...
        """
#        plt.hist(self.data)
        plt.hist(x = ['0', '1'], y= [(1 - self.p) * self.n, self.p * self.n])
        plt.title('Chart of data set')
        plt.ylabel('Value')
        plt.xlabel('Count')
    ...
    def pdf(self, k):
        ...
        """
        comb_nom = math.factorial(self.n)
        comb_denom = (math.factorial(k) * math.factorial(self.n-k))
        comb = comb_nom / comb_denom
    ...
    def plot_df(self):
        ...
        binom_dist = [self.pdf(i) for i in x_values] #y_axis

        fig = plt.figure()
        plt.bar(x_values, binom_dist)
        plt.title('pdf of binomial distribution')
        plt.ylabel('probability')
        plt.xlabel('no_success')
        plt.show()

        return x_values, binom_dist
...
#        result.stdev = math.sqrt(self.stdev ** 2 + other.stdev ** 2)
        ...
        result = Binomial(result.p, result.n)
    ...
    def __repr__magic(self):
        ...
        output = "mean {mu}, standard deviation {stdev}, p {p}, n{n}".format(mu=self.mean, stdev= self.stdev, p=self.p, n=self.n)
        return output
